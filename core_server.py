import sys
from os import chdir, getcwd, makedirs, mkdir, path, sep
...
async def ws_serve(websocket, path):
    ...
    try:
        async for data in websocket:
            ...
            if format_punc:
                try:
                    result_final = result_3 = await loop.run_in_executor(
                        None, punc_model, result_final
                    )
...
async def main():
    ...
    start_server = websockets.serve(
        ws_serve, addr, port, subprotocols=["binary"], max_size=None
    )
    try:
        await start_server
